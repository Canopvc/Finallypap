...
export default function HomeScreen() {
  ...
  const notificationsSentRef = useRef({
    half: false, target: false, double: false,
  });
    ...
    try {
      ...
      try {
        ...
        if (user?.id) {
          const isUuid =
            /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(user.id);
          let q = supabase.from("workouts").select("*").order("created_at", { ascending: false });
          q = isUuid ? q.eq("user_uuid", user.id) : q.eq("user_id", user.id);
      ...
      const unique = all.filter(
        (w, i, s) => i === s.findIndex((x) => x.name === w.name && x.createdAt === w.createdAt),
      );
  ...
  useFocusEffect(
    useCallback(() => { loadWorkouts(); }, [loadWorkouts]),
  );
  ...
  const checkStepMilestones = async () => {
    ...
    if (currentStepCount >= half && currentStepCount < half + 100 && !notificationsSentRef.current.half) {
      await sendMilestone("🎉 Metade do Caminho!", `Atingiste ${currentStepCount.toLocaleString()} passos! 💪`);
    ...
    if (currentStepCount >= STEP_TARGET && currentStepCount < STEP_TARGET + 100 && !notificationsSentRef.current.target) {
      await sendMilestone("🏆 Meta Batida!", `PARABÉNS! ${currentStepCount.toLocaleString()} passos! 🎊`);
      notificationsSentRef.current.target = true;
    }
    if (currentStepCount >= dbl && currentStepCount < dbl + 100 && !notificationsSentRef.current.double) {
      await sendMilestone("🚀 DOBRO DA META!", `INCRÍVEL! ${currentStepCount.toLocaleString()} passos! 🌟`);
  ...
  const registerForPushNotifications = async () => {
    ...
    if (status !== "granted") alert("We need your permission to send notifications");
  };
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        ...
        <Animated.View style={[styles.header, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          ...
          <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Soft); router.push("/profile"); }}>
            <Svg width={24} height={24} viewBox="0 0 24 24" fill="none" stroke={theme.colors.onSurface} strokeWidth={2}>
        ...
        {/* ── Steps Card ──────────────────────────────────────────────── */}
        <Animated.View style={[styles.stepsCard, { opacity: fadeAnim, transform: [{ translateY: slideAnim }], backgroundColor: theme.colors.surface, borderColor: theme.colors.outline + "30" }]}>
          <View style={styles.cardPadding}>
            ...
            <View style={styles.stepsCount}>
              ...
              <Text style={[styles.stepsDivider, { color: theme.colors.onSurfaceVariant }]}>/</Text>
            ...
            <View style={[styles.progressBar, { backgroundColor: theme.colors.surfaceVariant }]}>
              <View style={[styles.progressFill, { width: `${Math.min(100, progress * 100)}%`, backgroundColor: theme.colors.primary }]} />
            </View>
          </View>
        </Animated.View>

        {/* ── Workouts Section ────────────────────────────────────────── */}
        <Animated.View style={[styles.workoutsSection, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          <View style={styles.workoutsHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.onSurface }]}>
              {t("workouts", { ns: "common" })}
            </Text>
            <TouchableOpacity
              onPress={() => { clearAllWorkouts(); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Rigid); }}
              style={[styles.deleteBtn, { backgroundColor: theme.colors.error + "20", borderColor: theme.colors.error + "30" }]}
            >
            ...
            <AnimatedList
              ...
              renderItem={(item: Workout) => (
                <View style={styles.itemWrap}>
                  <TouchableOpacity
                    style={[styles.workoutItem, { backgroundColor: theme.colors.surface, borderColor: theme.colors.outline + "30" }]}
                    onPress={() => {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                      router.push(`/workout/${workoutSlugFromFields(item.name, item.createdAt)}`);
                    }}
                    activeOpacity={0.7}
                  >
                    <View style={styles.workoutContent}>
                      ...
                      <View style={[styles.exerciseCount, { backgroundColor: theme.colors.primary + "30" }]}>
                        <Text style={[styles.countText, { color: theme.colors.primary }]}>{item.exercises.length}</Text>
                      </View>
                    </View>
                  </TouchableOpacity>
                </View>
      ...
      <TouchableOpacity
        ...
        onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); router.push("/addWorkout"); }}
        activeOpacity={0.85}
      >
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingTop: Platform.OS === "ios" ? 60 : 50, paddingHorizontal: 24, paddingBottom: 20,
  },
  title:           { fontSize: 32, fontWeight: "700" },
  stepsCard:       { marginHorizontal: 24, marginBottom: 24, borderRadius: 24, overflow: "hidden", borderWidth: 1 },
  cardPadding:     { padding: 24 },
  stepsHeader:     { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16 },
  sectionTitle:    { fontSize: 18, fontWeight: "600" },
  caloriesBadge:   { flexDirection: "row", alignItems: "center", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, gap: 6 },
  ...
  stepsDivider:    { fontSize: 24, fontWeight: "600", marginHorizontal: 8, opacity: 0.6 },
  stepsTarget:     { fontSize: 24, fontWeight: "600", opacity: 0.8 },
  progressBar:     { height: 8, borderRadius: 4, overflow: "hidden" },
  progressFill:    { height: "100%", borderRadius: 4 },
  workoutsSection: { paddingHorizontal: 24 },
  workoutsHeader:  { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16 },
  ...
  workoutItem:     { borderRadius: 16, overflow: "hidden", borderWidth: 1, padding: 20 },
  workoutContent:  { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  workoutInfo:     { flex: 1 },
  workoutText:     { fontSize: 17, fontWeight: "600", marginBottom: 4 },
  dateText:        { fontSize: 14 },
  exerciseCount:   { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" },
  ...
  fab: {
    ...
    borderRadius: 30, overflow: "hidden", shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.4, shadowRadius: 12, elevation: 8,
  },
  fabGradient:     { width: "100%", height: "100%", alignItems: "center", justifyContent: "center" },
  fabText:         { fontSize: 32, fontWeight: "300" },
});